...
def generate_caption(post):
    ...
    # Only show qualities that have at least one link
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Try to clean intermediate messages
...
async def photo_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["to_delete"] = []
    track(context, update.message)  # user's photo
...
async def type_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # edit keeps same message_id, already tracked
...
async def title_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    track(context, update.message)  # user's title
    post = get_user_data(context)
...
async def sample_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if post["type"] == "episode":
        msg = await update.message.reply_text("📺 Season number bhejo (jaise 1 ya 01):")
        track(context, msg)
...
async def season_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("🎞 Episode number bhejo (jaise 5 ya 05):")
    track(context, msg)
...
async def new_quality_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_q in post["qualities"]:
        msg = await update.message.reply_text("⚠️ Yeh quality pehle se hai. Koi aur naam do.")
        track(context, msg)
        return WAITING_NEW_QUALITY
...
async def confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "confirm_yes":
        ...
        # Delete ALL intermediate messages (photo, inputs, preview, buttons etc.)
        to_delete = context.user_data.get("to_delete", [])
        # Also delete the current confirm message
