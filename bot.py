...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📌 *Kaise use karein:*\n"
        "• Photo bhejo *ya* `/new` command\n"
        "• Type select karo (Movie / Web Series / Episode)\n"
        "• Details + quality links daalo\n"
        "• Web Series mein multiple seasons add kar sakte ho\n\n"
        "Commands:\n"
        "/new - Bina photo ke naya post\n"
        "/cancel - Cancel\n"
        "/addadmin <id> - Admin add (owner only)\n"
        "/removeadmin <id>\n"
        "/admins\n"
        "/help",
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🛠 *SKxPoster Help*\n\n"
        "1. Photo bhejo *ya* `/new`\n"
        "2. Type select\n"
        "3. Title → Sample (opt) → Stream (opt)\n"
        "4. Web Series: Season number → qualities → next season\n"
        "5. Finish → Hashtags (opt) → Confirm\n\n"
        "Sirf unhi qualities dikhengi jisme link add kiya hoga.",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        ...
        await update.message.reply_text("❌ Cancelled. Naya post ke liye photo bhejo ya /new")
...
async def ask_stream(update, context, from_callback=False):
    keyboard = [
        [InlineKeyboardButton("⏭ Skip Stream/Watch Link", callback_data="skip_stream")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel")],
    ]
    text = "▶️ Stream / Watch Online Link bhejo (optional):"
    if from_callback:
