...
# Full DMCA poster image (clickable)
...
(
    ...
    WAITING_SAMPLE,
    WAITING_TRAILER,
    WAITING_STREAM,          # first stream link
    WAITING_MORE_STREAM,     # additional stream links
    WAITING_SEASON,
    WAITING_EPISODE,
    WAITING_SEASON_NUM,
    QUALITY_MENU,
...
def get_user_data(context: ContextTypes.DEFAULT_TYPE):
    if "post" not in context.user_data:
        context.user_data["post"] = {
            ...
            "title": None,
            "sample": None,
            "trailer": None,             # single trailer link
            "streams": [],               # multiple watch/stream links
            "season": None,
            "episode": None,
            "seasons": {},
...
def generate_caption(post):
    """
    Smart caption:
    1. Try full stylish version
    2. If too long → remove Channels section
    3. If still too long → use ultra compact version
    """
    MAX_LEN = 1020  # safe under 1024

    def build_full(include_channels=True):
        lines = []
        ...
        if post.get("streams"):
            ...
            for idx, link in enumerate(post["streams"], 1):
                lines.append(f'🔗 <a href="{link}">Stream Link {idx}</a>')
            lines.append("")
            lines.append("⚠️ <i>Note: Online streams may contain ads</i>")
            lines.append("")
            ...
            for season_num in sorted(post["seasons"].keys(), key=lambda x: int(x) if str(x).isdigit() else 0):
                ...
                for q_name, q_data in qualities.items():
                    if q_data["links"]:
                        ...
                        size = q_data["size"] if q_data.get("size") else "—"
        ...
        # DMCA always kept
        lines.append(f'⚖️ <a href="{DMCA_IMAGE_URL}"><b>𝗗𝗠𝗖𝗔 & 𝗖𝗢𝗡𝗧𝗘𝗡𝗧 𝗗𝗜𝗦𝗖𝗟𝗔𝗜𝗠𝗘𝗥</b></a>')
    ...
    def build_compact():
        ...
        if post["type"] == "episode" and post.get("season") and post.get("episode"):
            lines.append(f"📺 Season {post['season']} • Episode {post['episode']}")

        lines.append("")

        if post.get("sample"):
            lines.append(f'🎞️ Sample: <a href="{post["sample"]}">Link</a>')

        if post.get("trailer"):
            lines.append(f'🎬 Trailer: <a href="{post["trailer"]}">Watch</a>')

        if post.get("streams"):
            lines.append("▶️ <b>Watch / Stream</b>")
            for idx, link in enumerate(post["streams"], 1):
                lines.append(f'🔗 <a href="{link}">Stream {idx}</a>')
            lines.append("<i>⚠️ Streams may contain ads</i>")
        ...
        if post["type"] == "webseries" and post.get("seasons"):
            for season_num in sorted(post["seasons"].keys(), key=lambda x: int(x) if str(x).isdigit() else 0):
                ...
                if not has_any:
                    continue
                lines.append(f"<b>Season {season_num}</b>")
                for q_name, q_data in qualities.items():
                    if q_data["links"]:
                        size = f" ({q_data['size']})" if q_data.get("size") else ""
                        links_txt = " | ".join([f'<a href="{l}">Link{i}</a>' for i, l in enumerate(q_data["links"], 1)])
                        lines.append(f"🔹 {q_name}{size}: {links_txt}")
                lines.append("")
        else:
            for q_name, q_data in post["qualities"].items():
                if q_data["links"]:
        ...
        lines.append("")
        lines.append("━━━━━━━━━━━━━━━━")
        lines.append(f'⚖️ <a href="{DMCA_IMAGE_URL}"><b>DMCA & Disclaimer</b></a>')
        lines.append("Takedown → @SKxMOVIES_RequestBot")
    ...
    # 1. Try full stylish
    caption = build_full(include_channels=True)
    if len(caption) <= MAX_LEN:
        return caption

    # 2. Try without Channels section
    caption = build_full(include_channels=False)
    if len(caption) <= MAX_LEN:
        return caption

    # 3. Fallback to ultra compact
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📌 *Kaise use karein:*\n"
        "• Photo / Poster URL / `/new`\n"
        "• Type → Title → Sample → Trailer → Stream(s)\n"
        "• Web Series: multiple seasons\n"
        "• Multiple Stream links supported\n\n"
        "Commands:\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🛠 *SKxPoster Help*\n\n"
        "1. Photo upload *ya* Poster URL *ya* `/new`\n"
        "2. Type select\n"
        "3. Title → Sample → Trailer → Stream links (multiple)\n"
        "4. Web Series: Season by Season\n"
        "5. Finish → Hashtags → Confirm\n\n"
...
async def poster_url_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_image_url(text):
        post["poster"] = text
        msg = await update.message.reply_text("✅ Poster URL save.\n\n📝 Ab *Title* bhejo:", parse_mode=ParseMode.MARKDOWN)
    else:
        msg = await update.message.reply_text("⚠️ Valid image URL nahi. Phir se bhejo ya Skip dabao.")
        track(context, msg)
        return WAITING_POSTER_URL
...
async def title_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            await update.message.reply_text(
                "⚠️ Kuch error aa gaya. Dobara Title bhejo ya /cancel karke naya start karo."
            )
        except Exception:
            pass
        return WAITING_TITLE
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            WAITING_MORE_STREAM: [
                CallbackQueryHandler(more_stream_handler),
            ],
