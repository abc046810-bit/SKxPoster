...
def generate_caption(post):
    ...
    # Multiple Stream / Watch links
    if post.get("streams"):
        lines.append("▶️ <b>𝗪𝗔𝗧𝗖𝗛 / 𝗦𝗧𝗥𝗘𝗔𝗠</b>")
        for idx, link in enumerate(post["streams"], 1):
            lines.append(f'🔗 <a href="{link}">Stream Link {idx}</a>')
        lines.append("")
        lines.append("⚠️ <i>Note: Online streams may contain ads (few or many). We do not control third-party players or ads.</i>")
        lines.append("")
    ...
    # DMCA - short + clickable full poster
    lines.append(f'⚖️ <a href="{DMCA_IMAGE_URL}"><b>𝗗𝗠𝗖𝗔 & 𝗖𝗢𝗡𝗧𝗘𝗡𝗧 𝗗𝗜𝗦𝗖𝗟𝗔𝗜𝗠𝗘𝗥</b></a>')
    lines.append("")
    lines.append("We do not host or store any files. All links are from third-party sources.")
    lines.append("For takedown requests → @SKxMOVIES_RequestBot")
    lines.append("")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append("")
